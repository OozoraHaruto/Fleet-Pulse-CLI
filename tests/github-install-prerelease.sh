...
cat >"$release_dir/releases.json" <<JSON
[
  {
    "tag_name": "$version",
    "prerelease": true,
...
case "$url" in
  https://api.github.com/repos/OozoraHaruto/Fleet-Pulse-CLI/releases/latest)
    exit 22
    ;;
  https://api.github.com/repos/OozoraHaruto/Fleet-Pulse-CLI/releases)
    src="$FLEETPULSE_TEST_RELEASE_DIR/releases.json"
    ;;
...
PATH="$bin_dir:$PATH" \
  FLEETPULSE_ALLOW_PRERELEASE=true \
